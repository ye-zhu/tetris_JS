...
/***/ function(module, exports, __webpack_require__) {
	...
	var View = function (_React$Component) {
	  ...
	  _createClass(View, [{
	    ...
	    value: function render() {
	      ...
	      var piece = this.state.game.gamePiece;
	      var board = this.state.game.board.grid.map(function (row, rowidx) {
	        var units = row.map(function (unit, colidx) {
	          ...
	          _this2.state.game.gamePiece.pos.forEach(function (el) {
	            if (el[0] === rowidx && el[1] === colidx) {
	              additionClass = _this2.state.game.gamePiece.fillColor;
	            }
	          });
	          ...
	          return _react2.default.createElement('div', { className: 'unit ' + additionClass + ' ' + unit.filled, key: '' + colidx });
	        });
	...
	document.addEventListener("DOMContentLoaded", function () {
	  ...
	  _reactDom2.default.render(_react2.default.createElement(View, null), root);
	});
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Game = function () {
	  function Game(view) {
	    ...
	    this.gamePiece = new _piece2.default();
	  ...
	  _createClass(Game, [{
	    key: 'stopPiece',
	    value: function stopPiece() {
	      var _this = this;

	      var collided = this.gamePiece.pos.filter(function (pos) {
	        if (pos[0] >= 0) {
	          return _this.board.grid[pos[0] + 1][pos[1]].filled;
	        }
	      });

	      if (collided.length > 0) {
	        this.gamePiece.pos.forEach(function (pos) {
	          _this.board.grid[pos[0]][pos[1]].filled = _this.gamePiece.fillColor;
	        });
	        this.gamePiece = new _piece2.default();
	        this.clearRow();
	      }
	    ...
	    key: 'clearRow',
	    value: function clearRow() {
	      var _this2 = this;

	      this.board.grid.forEach(function (row, rowidx) {
	        if (row.filter(function (pos) {
	          return pos.filled && pos.filled !== "black";
	        }).length === 10) {
	          _this2.board.grid.splice(rowidx, 1);
	          _this2.board.grid.unshift(_this2.board.units());
	          _this2.score += 100;
	        }
	      });
	    }
	    ...
	    key: 'movePiece',
	    value: function movePiece(delta) {
	      this.gamePiece.pos = this.gamePiece.pos.map(function (pos) {
	    ...
	    key: 'rotatePiece',
	    value: function rotatePiece(matrix) {
	      var center = this.gamePiece.pos[1];
	      var relativePos = this.gamePiece.pos.map(function (coord) {
	    ...
	    key: 'setPieceIntoMotion',
	    value: function setPieceIntoMotion() {
	      var downwardMovement = function downwardMovement() {
	        this.movePiece([1, 0]);
	        this.validMove([1, 0]);
	    ...
	    key: 'validMove',
	    value: function validMove(delta) {
	      var _this3 = this;

	      var proxyMove = this.gamePiece.pos.map(function (pos) {
	        return [pos[0] + delta[0], pos[1] + delta[1]];
	      });

	      var startPos = 0;
	      for (var i = 0; i < this.gamePiece.pos.length; i += 1) {
	        var gp = this.gamePiece.pos[i];
	        var sp = this.gamePiece.startPos[i];
	        if (gp[0] === sp[0] && gp[1] === sp[1]) {
	          startPos += 1;
	        }
	      }

	      var check = proxyMove.filter(function (pos) {
	        return pos[0] > 0 && _this3.board.grid[pos[0]][pos[1]].filled !== false;
	      });
	      // GAME OVER LOGIC //
	    ...
	    key: 'rotateValidMove',
	    value: function rotateValidMove(rotatedPos) {
	      var _this4 = this;

	      if (rotatedPos.filter(function (pos) {
	        return pos[1] > 13 || pos[1] < 0;
	      }).length > 0) {
	        return false;
	      } else {
	        var check = rotatedPos.filter(function (pos) {
	          return _this4.board.grid[pos[0]][pos[1]].filled;
	        });
	        if (check.length > 0) {
	          return false;
	        } else {
	    ...
	    key: 'sortAndMakeUnique',
	    value: function sortAndMakeUnique() {}
	  }, {
	    key: 'rotateCheck',
	    value: function rotateCheck(rotatePos) {
	      var _this5 = this;

	      var filledPos = [];
	      var tempPos = [];
	      rotatePos.map(function (pos) {
	        tempPos.push(pos[1]);
	        if (_this5.board.grid[pos[0]][pos[1]].filled) {
	      ...
	      if (filledPos.length > 0) {
	        (function () {
	          var sortAndUnique = function sortAndUnique(arr) {
	            var set = new Set(arr.sort(function (a, b) {
	              return a > b;
	            }));
	            var uniqueSorted = [];
	          ...
	          var filledCompare = sortAndUnique(filledPos);
	          var tempPosCompare = sortAndUnique(tempPos);
	          var rotatedPiece = void 0;

	          if (filledCompare[0] === tempPosCompare[0]) {
	            rotatedPiece = rotatePos.map(function (pos) {
	          ...
	          if (rotatedPiece && _this5.rotateValidMove(rotatedPiece)) {
	            _this5.gamePiece.pos = rotatedPiece;
	            _this5.stopPiece();
	          } else {
	            // Nothing Happens
	          }
	    ...
	    value: function pauseGame() {
	      ...
	      document.addEventListener('keydown', function (e) {
	        if (e.key === 'g' && _this6.running) {
	          _this6.running = false;
	          document.removeEventListener('keydown', _this6.listener);
	          clearInterval(_this6.interval);
	        } else if (e.key === 'g') {
	          _this6.running = true;
	          _this6.addListenders();
	          _this6.setPieceIntoMotion();
	        }
	      });
	    }
	  }, {
	    key: 'gameOver',
	    value: function gameOver(newPiece) {
